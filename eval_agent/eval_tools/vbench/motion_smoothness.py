...
from vbench.utils import load_dimension_info, CACHE_DIR
...
def motion_smoothness(motion, video_pairs):
    sim = []
    video_results = []
...
def compute_motion_smoothness(video_pairs):
    device = torch.device("cuda")
    
    CUR_DIR = os.path.dirname(os.path.abspath(__file__))
    config = f'{CUR_DIR}/third_party/amt/cfgs/AMT-S.yaml' # pretrained/amt_model/AMT-S.yaml
    ckpt = f'{CACHE_DIR}/amt_model/amt-s.pth' # pretrained/amt_model/amt-s.pth
    
    motion = MotionSmoothness(config, ckpt, device)

    results = motion_smoothness(motion, video_pairs)
    return results
