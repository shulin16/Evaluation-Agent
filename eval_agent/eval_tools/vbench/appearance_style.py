...
def appearance_style(clip_model, video_pairs, device):
    sim = 0.0
    cnt = 0
    video_results = []
    ...
    for info in tqdm(video_pairs):
        ...
        with torch.no_grad():
            ...
            video_results.append({'prompt':prompt, 'video_path': video_path, 'video_results': video_sim})
...
def compute_appearance_style(video_pairs):
    device = torch.device("cuda")
    submodules_list = {"name": 'ViT-B/32'}
    clip_model, preprocess = clip.load(device=device, **submodules_list)
    results = appearance_style(clip_model, video_pairs, device)
    return results
